...
\UCitem{Salidas}{
	Se muestra el mensaje \textbf{MSG-42}: ``¿Confirmar aprobación? ¿Está seguro de aprobar esta solicitud y asignar al docente como reemplazo?'', solicitando confirmación para la asignación del reemplazo.
	
	Se muestra el mensaje \textbf{MSG-43}: ``Motivo de rechazo. Por favor, indique el motivo del rechazo'', indicando que se requiere justificación para denegar la solicitud.}
...
\UCitem{Errores}{
	\begin{itemize}
		\item \textbf{E1}: Pérdida de conexión. Muestra el mensaje \textbf{MSG-41}: ``Error de red. Verifique su conexión e intente nuevamente''.
		\item \textbf{E2}: Datos inválidos. Muestra \textbf{MSG-42}: ``Los datos recibidos no tienen el formato esperado'' (cuando faltan campos obligatorios).
		\item \textbf{E3}: Error en aprobación/rechazo. Muestra \textbf{MSG-43}: ``No se pudo completar la acción. Intente más tarde''.
		\item \textbf{E4}: Docentes no disponibles. Muestra \textbf{MSG-44}: ``Error al cargar docentes''.
	\end{itemize}
}
...
\begin{UCtrayectoriaA}{C}{Rechazar solicitud}
	\UCpaso El sistema muestra el campo \IUref{IU12}{Ingresar motivo de rechazo}.
